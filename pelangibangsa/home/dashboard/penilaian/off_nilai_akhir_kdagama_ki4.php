<?php 
$sql = mysqli_query($koneksi, "SELECT id_raportkd, Z.id_siswa as id_siswa, nama_siswa, X.id_mapel as id_mapel, X.nama_mapel as nama_mapel, C.id_agama as id_agama, Y.id_kd as id_kd, Y.nomor_kd as nomor_kd, Y.deskripsi as deskripsi, tugas1, tugas2, tugas3, tugas4, tugas5, ph1, ph2, ph3, ph4, pts, pas FROM nilai_agamakd_ki4 B RIGHT JOIN mapel X ON X.id_mapel = B.id_mapel
RIGHT JOIN kd_agama_ki4 Y ON Y.id_mapel = X.id_mapel INNER JOIN agama C ON C.id_agama = Y.id_agama INNER JOIN data_siswa Z WHERE Z.id_siswa = '".$_GET['sis']."' AND Y.id_agama = '".$_GET['agm']."' GROUP BY Z.id_agama");
...
<div class="card">
	...
	<div class="body">
		...
		<table class="table table-responsive">
			...
			$query = mysqli_query($koneksi, "SELECT id_raportkd, Z.id_siswa as id_siswa , X.id_mapel as id_mapel, Y.id_kelas as id_kelas, Y.id_semester as id_semester , X.nama_mapel as nama_mapel, C.id_agama as id_agama, Y.id_kd as id_kd, Y.nomor_kd as nomor_kd, Y.deskripsi as deskripsi, tugas1, tugas2, tugas3, tugas4, tugas5, ph1, ph2, ph3, ph4, pts, pas FROM nilai_agamakd_ki4 B RIGHT JOIN mapel X ON X.id_mapel = B.id_mapel RIGHT JOIN kd_agama_ki4 Y ON Y.id_mapel = X.id_mapel INNER JOIN agama C ON C.id_agama = Y.id_agama INNER JOIN data_siswa Z WHERE Z.id_siswa = '".$data['id_siswa']."' AND Y.id_agama = '".$data['id_agama']."' AND Y.id_mapel = '".$data['id_mapel']."' AND Y.id_kelas = '".$dtsq1['id_kelas']."' AND Y.id_semester = '".$dtsq2['id_semester']."' GROUP BY Y.id_kd");
							...
							<td>
								<div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        <li><a href="?page=nilai_akhir_kdagamaki4&id=<?= $data['id_mapel'] ?>&sis=<?= $data['id_siswa']; ?>&agm=<?= $kd['id_agama']; ?>&kd=<?= $kd['id_kd']; ?>">Nilai</a></li>
                                    </ul>
                                </div>
							</td>
						</tr>
		...
		<table class="table table-responsive table-bordered table-hover">
			...
			$sqlkd = mysqli_query($koneksi, "SELECT * FROM hitung_kdagm_ki4 WHERE id_mapel = '".$data['id_mapel']."' AND id_siswa = '".$data['id_siswa']."' AND id_kelas = '".$dtsq1['id_kelas']."' AND id_semester = '".$dtsq2['id_semester']."'");
					...
					<tr class="font-12">
						...
						<td>
							<div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        <li><a href="?page=nilai_akhir_kdagamaki4&id=<?= $data['id_mapel']; ?>&sis=<?= $data['id_siswa']; ?>&agm=<?= $data['id_agama'] ?>&edit=<?= $kd['id_kd']; ?>">Edit</a></li>
                                    </ul>
                                </div>
						</td>
					</tr>
...
if (isset($_GET['kd'])) {
	include "input_akhir_agama_ki4.php";
}
if(isset($_GET['edit'])){
	include "edit/edit_nilai_akhir_agamaki4.php";
}else{
	echo "";
}
?>
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <?php $nilkd = mysqli_query($koneksi, "SELECT max(nilai_kd) as nilai FROM hitung_kdagm_ki4 WHERE id_mapel = '".$data['id_mapel']."' AND id_siswa ='".$data['id_siswa']."'");
                            $datakd = mysqli_fetch_array($nilkd);


                            $sqlkkm = mysqli_query($koneksi, "SELECT Y.keterampilan as keterampilan, Y.id_mapel as id_mapel, id_kelas, id_semester FROM hitung_kdagm_ki4 X INNER JOIN mapel Y WHERE Y.id_mapel ='".$data['id_mapel']."' AND id_siswa = '".$data['id_siswa']."' AND id_kelas = '".$dtsq1['id_kelas']."' AND id_semester = '".$dtsq2['id_semester']."'");
                            ...
                            ?>        
                                        ...
                                        <div class="col-lg-3 col-md-12 col-sm-12 col-xs-12">
                                            <div class="form-group form-float">
                                                <div class="form-line">
                                                    <label class="form-label">ID Mapel</label>
                                                    <input type="text" class="form-control" name="id_kelas" value="<?= $datakkm['id_kelas']; ?>" required readonly>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-lg-3 col-md-12 col-sm-12 col-xs-12">
                                            <div class="form-group form-float">
                                                <div class="form-line">
                                                    <label class="form-label">ID Mapel</label>
                                                    <input type="text" class="form-control" name="id_semester" value="<?= $datakkm['id_semester']; ?>" required readonly>
                                                </div>
                                            </div>
                                        </div>
                                        </div>
                                            ...
                                            <div class="form-group form-float">
                                                <div class="form-line">
                                                    ...
                                                    <textarea name="deskripsi" cols="30" rows="5" class="form-control no-resize" required readonly><?php echo $sis . $sttm . "tuntas dengan predikat " . $stm . ", " . $sis ." ". $stm . " dalam "; kkmagmki4(); ?></textarea>
                                                </div>
                                            </div>
                                            <?php 
