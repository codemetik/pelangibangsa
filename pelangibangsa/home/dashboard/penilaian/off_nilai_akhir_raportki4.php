<?php 
$sql = mysqli_query($koneksi, "SELECT* FROM nilai_akhir_raportkd_ki4 WHERE id_mapel = '".$_GET['id']."' AND id_siswa = '".$_GET['sis']."'");
...
<div class="card">
	<div class="header bg-indigo">
		<h2>Penilaian akhir kd Mapel : <?= $data['nama_mapel']; ?></h2>
	</div>
	<div class="body">
		<div class="col-lg-12 col-md-6 col-sm-6 col-xs-6">
			<div class="form-line">
				<input type="text" class="form-control" value="<?= $dtview['nama_siswa']; ?>" readonly>
			</div>
		</div>
		<table class="table table-responsive">
			...
			$query = mysqli_query($koneksi, "SELECT * FROM nilai_akhir_raportkd_ki4 
				WHERE id_mapel = '".$data['id_mapel']."' AND id_siswa = '".$data['id_siswa']."' AND id_kelas = '".$dtsq1['id_kelas']."' AND id_semester = '".$dtsq2['id_semester']."' GROUP BY id_kd");
				while ($kd = mysqli_fetch_array($query)) { ?>
					<tbody class="font-12">
						<tr>
							...
							<td>
				               <textarea name="deskripsi" cols="30" rows="3" class="form-control no-resize" required><?= $kd['deskripsi']; ?></textarea>
				            </td>
							<td>
								<div class="dropdown">
                                    <button type="submit" href="" class="btn bg-indigo" data-toggle="dropdown">Action</button>
                                    <ul class="dropdown-menu pull-right">
                                        <li><a href="?page=nilai_akhir_raportki4&id=<?= $data['id_mapel'] ?>&sis=<?= $data['id_siswa']; ?>&kd=<?= $kd['id_kd']; ?>">Nilai</a></li>
                                    </ul>
                                </div>
							</td>
						</tr>
	...
	<div class="table table-responsive">
		<table class="table table-responsive table-bordered table-hover">
			...
			<?php 
			$query = mysqli_query($koneksi, "SELECT * FROM hitung_kdki4 WHERE id_mapel = '".$data['id_mapel']."' AND id_siswa = '".$data['id_siswa']."' AND id_kelas = '".$dtsq1['id_kelas']."' AND id_semester = '".$dtsq2['id_semester']."' ");
			while ($kd = mysqli_fetch_array($query)) { ?>
				<tbody>
					<tr class="font-12">
						...
						<td>
							<div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        <li><a href="?page=nilai_akhir_raportki4&id=<?= $data['id_mapel']; ?>&sis=<?= $data['id_siswa']; ?>&edit=<?= $kd['id_kd']; ?>">Edit</a></li>
                                    </ul>
                                </div>
						</td>
					</tr>
		...
		<?php 
		$sqlcek = mysqli_query($koneksi, "SELECT * FROM nilai_raport_ki4 WHERE id_mapel = '".$data['id_mapel']."' AND id_siswa = '".$data['id_siswa']."' AND id_kelas = '".$dtsq1['id_kelas']."' AND id_semester = '".$dtsq2['id_semester']."'");
		$cekmpl = mysqli_num_rows($sqlcek);
		if ($cekmpl == 1) {
		    echo "<div class='pull-right'><a href='?page=nilai&id=".$dtview['nama_siswa']."'><button class='btn btn-success'><h4>Nilai sudah keluar</h4></button></a></div>";
		}else{
		    echo "<div class='pull-right'><a href='?page=nilai&id=".$dtview['nama_siswa']."'><button class='btn btn-danger'><h4>Nilai belum keluar</h4></button></a></div>";
		}
		?>
	</div>
		<form>
			<!-- index.php?page=nilai&id=<?= $data['id_siswa']; ?> -->
...
if (isset($_GET['kd'])) {
	include "input_akhir_raport_ki4.php";
}
if(isset($_GET['edit'])){
	include "edit/edit_nilai_akhir_raportki4.php";
}else{
	echo "";
}
?>
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title" id="defaultModalLabel">Nilai : <?= $data['id_mapel']; ?> Atas Nama : <?= $dtview['nama_siswa']; ?></h4>
                        </div>
                        <div class="modal-body">
                            <div class="body">
                            <div class="row clearfix">
                                <div class="col-xs-12 ol-sm-12 col-md-12 col-lg-12">
                            <?php $nilkd = mysqli_query($koneksi, "SELECT max(nilai_kd) as nilai FROM hitung_kdki4 WHERE id_mapel = '".$data['id_mapel']."' AND id_siswa = '".$data['id_siswa']."' AND id_kelas = '".$dtsq1['id_kelas']."' AND id_semester = '".$dtsq2['id_semester']."'");
                            $datakd = mysqli_fetch_array($nilkd);

                            $sqlkkm = mysqli_query($koneksi, "SELECT Y.keterampilan as keterampilan, Y.id_mapel as id_mapel, id_kelas, id_semester FROM hitung_kdki4 X INNER JOIN mapel Y WHERE Y.id_mapel ='".$data['id_mapel']."' AND id_siswa = '".$data['id_siswa']."' AND id_kelas = '".$dtsq1['id_kelas']."' AND id_semester = '".$dtsq2['id_semester']."' ");
										$datakkm = mysqli_fetch_array($sqlkkm);

										$nilaimax = $datakd['nilai'];
										$kkm = $datakkm['keterampilan'];
										...
										*/
										...
										$km1 = $datakkm['keterampilan'];
										include "ambilnilaisiswa.php";
                            			?>        
                            
                                        <form id="form_validation" method="POST" action="proses/simpan_hasilki4.php">
                                        <div class="row">
                                        ...
                                        <div class="form-group form-float">
                                                <div class="form-line">
                                                    ...
                                                    <input type="text" class="form-control" name="nama_siswa" value="<?= $dtview['nama_siswa']; ?>" required readonly>
                                                </div>
                                            </div>
                                            <div class="form-group form-float">
                                                <div class="form-line">
                                            ...
                                            <div class="form-group form-float">
                                                <div class="form-line">
                                                    ...
                                                    <textarea name="deskripsi" cols="30" rows="5" class="form-control no-resize" required readonly><?php echo $sis . $sttm . "tuntas dengan predikat " . $stm . ", " . $sis ." ". $stm . " dalam "; kkmki4(); ?></textarea>
                                                </div>
                                            </div>
                                            <?php 
