...
$tgl_raport = $_POST['tgl_raport'];
	...
	$sqlcek = mysqli_query($koneksi, "SELECT * FROM nilai_raport_ki4 WHERE id_siswa = '".$id_siswa."' AND id_mapel = '".$id_mapel."' AND id_kelas = '".$id_kelas."' AND id_semester = '".$id_semester."'");
	$datacek = mysqli_num_rows($sqlcek);
if ($datacek == 1) {
	echo "<script>
	alert('Silahkan hapus nilai KETERAMPILAN sebelumnya, jika ingin update nilai mapel terbaru.');
	document.location.href = '../?page=nilai&id=".$nama_siswa."';
	</script>";
}else{

	$kd = mysqli_query($koneksi, "SELECT max(id_nilairaport) AS maxCode FROM nilai_raport_ki4");
