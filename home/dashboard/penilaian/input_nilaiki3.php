...
$sql_kd = mysqli_query($koneksi, "SELECT * FROM kd_pengetahuan WHERE id_mapel = '".$_GET['id']."' AND id_kelas = '".$dtsq1['id_kelas']."' AND id_semester = '".$dtsq2['id_semester']."'");
$cek_mapel = mysqli_num_rows($sql_kd);
if ($cek_mapel > 0) {
	...
	$array[] = array(
		'id_kd' => 'KD Mapel tidak ada',
		'nomor_kd' => 'KD Mapel tidak ada',
		'deskripsi' => 'KD Mapel tidak ada',
		'id_mapel' => 'KD Mapel tidak ada',
		'id_kelas' => 'KD Mapel tidak ada',
		'id_semester' => 'KD Mapel tidak ada'
	);
}


$sql_siswa = mysqli_query($koneksi, "SELECT * FROM data_siswa WHERE id_siswa = '".$_GET['sis']."'");
$dt_siswa = mysqli_fetch_array($sql_siswa);

// echo $dtsq1['id_kelas']." dan ".$dtsq2['id_semester'];

$sql_kkm = mysqli_query($koneksi, "SELECT id_mapel, nama_mapel ,pengetahuan , 
...
$data_kkm = mysqli_fetch_array($sql_kkm);

// echo $data_kkm['id_mapel'];
?>
<div class="card">
	<div class="header bg-indigo">
		<h2>INPUT NILAI KD PENGETAHUAN <?= $array[0]['id_mapel']; ?> | <?= $dt_siswa['nama_siswa']; ?></h2>
	</div>
	<div class="body">
		<form accept="" method="POST">
			<div class="row">
				...
				<div class="col-sm-12">
                    ...
                    <?php 
                    if ($array[0]['id_mapel'] == 'KD Mapel tidak ada') { ?>
                    	<input type="text" class="form-control bg-red" name="nama_mapel" value="<?= $array[0]['id_mapel']; ?>" required readonly>
                    <?php }else{ ?>
                    	<input type="text" class="form-control bg-blue-grey" name="nama_mapel" value="<?= $data_kkm['nama_mapel']; ?>" required readonly>
                    <?php }
                    ?>
                </div>
				</div>
...
if (isset($_POST['tampil'])) {
	...
	if ($min > $data_kkm['baik']) {
		$des1 = "Sangat baik";
	}else if ($min > $data_kkm['cbaik']) {
		$des1 = "Baik";
	}else if ($min >= $data_kkm['pbimbingan']) {
		$des1 = "Cukup baik";
	}else{
	...
	if ($max > $data_kkm['baik']) {
		$des2 = "Sangat baik";
	}else if ($max > $data_kkm['cbaik']) {
		$des2 = "Baik";
	}else if ($max >= $data_kkm['pbimbingan']) {
		$des2 = "Cukup baik";
	}else{
	...
	//mencari predikat
	if ($nilai > $data_kkm['baik']) {
		$predikat = "A";
	}else if ($nilai > $data_kkm['cbaik']) {
		$predikat = "B";
	}else if ($nilai >= $data_kkm['pbimbingan']) {
		$predikat = "C";
	}else{
		$predikat = "D";
	}
	?>
<form action="penilaian/proses_nilai_akhirki3.php" method="POST">
		...
		<table class="table table-responsive table-hover table-bordered">
			...
			<tbody>
				<tr>
					...
					<td ><?= $min; ?>
						<div hidden><input type="text" name="min" value="<?= $min; ?>"></div>
					</td>
					<td width="100px"><?= $des1; ?>
						<div hidden><input type="text" name="des1" value="<?= $des1; ?>"></div>
					</td>
					<td><?= $max; ?>
						<div hidden><input type="text" name="max" value="<?= $max; ?>"></div>
					</td>
					<td width="100px"><?= $des2; ?>
						<div hidden><input type="text" name="des2" value="<?= $des2; ?>"></div>
					</td>
					<td><?= round($avg); ?>
						<div hidden><input type="text" name="avg" value="<?= round($avg); ?>"></div>
					</td>
					<td><?= round($nilai); ?>
						<div hidden><input type="tet" name="nilai" value="<?= round($nilai); ?>"></div>
					</td>
					<td><?= $predikat; ?>
