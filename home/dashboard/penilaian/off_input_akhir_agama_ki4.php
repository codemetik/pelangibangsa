<?php 
$sql = mysqli_query($koneksi, "SELECT id_raportkd, Z.id_siswa as id_siswa , X.id_mapel as id_mapel, X.nama_mapel as nama_mapel, C.id_agama as id_agama, Y.id_kd as id_kd, Y.nomor_kd as nomor_kd, Y.deskripsi as deskripsi, tugas1, tugas2, tugas3, tugas4, tugas5, ph1, ph2, ph3, ph4, pts, pas FROM nilai_agamakd_ki4 B RIGHT JOIN mapel X ON X.id_mapel = B.id_mapel RIGHT JOIN kd_agama_ki4 Y ON Y.id_mapel = X.id_mapel INNER JOIN agama C ON C.id_agama = Y.id_agama INNER JOIN data_siswa Z WHERE Z.id_siswa = '".$_GET['sis']."' AND Y.id_agama = '".$_GET['agm']."' AND Y.id_kd = '".$_GET['kd']."' AND Y.id_mapel = '".$_GET['id']."' GROUP BY Y.id_kd");
$dat = mysqli_fetch_array($sql);

//ambil data maximal di kolom id_raportkd table nilai_raportkd_ki3
$queryku = mysqli_query($koneksi, "SELECT max(id_raportkd) AS maxCode FROM nilai_agamakd_ki4");
...
$sqli = mysqli_query($koneksi, "SELECT * FROM nilai_agamakd_ki4 WHERE id_siswa = '".$id_siswa."' AND id_mapel = '".$id_mapel."' AND id_kd = '".$id_kd."'");
$cek = mysqli_num_rows($sqli);
    if ($cek == 1) {
        echo "<script>
        alert('Data sudah ada');
        document.location.href = '?page=nilai_akhir_kdagamaki4&id=$id_mapel&sis=$id_siswa&agm=$id_agama';
        </script>";
    }else{

     $isi = mysqli_query($koneksi, "INSERT INTO nilai_agamakd_ki4(id_raportkd, id_siswa, id_mapel, id_kelas, id_semester, nama_mapel, id_kd, nomor_kd, deskripsi, tugas1, tugas2, tugas3, tugas4, tugas5, ph1, ph2, ph3, ph4, pts, pas) VALUES('".$id_raportkd."','".$id_siswa."','".$id_mapel."', '".$id_kelas."', '".$id_semester."','".$nama_mapel."','".$id_kd."','".$nomor_kd."','".$deskripsi."','".$tugas1."','".$tugas2."','".$tugas3."','".$tugas4."','".$tugas5."','".$ph1."','".$ph2."','".$ph3."','".$ph4."','".$pts."','".$pas."')");
     if ($isi) {
        echo "<script>
