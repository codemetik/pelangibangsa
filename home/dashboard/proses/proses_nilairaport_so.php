...
if (isset($_POST['submit'])) {
	...
	$idrpt = $_POST['id_nilairaport_so'];
	$id_kelas = $_POST['id_kelas'];
	$id_semester = $_POST['id_semester'];
	$tgl_raport = $_POST['tgl_raport1'];
	$tahun_awal = $_POST['tahun_awal'];
	$tahun_akhir = $_POST['tahun_akhir'];
	

$sql = mysqli_query($koneksi, "SELECT * FROM nilai_raport_sosial WHERE id_siswa = '".$idsiswa."' AND id_kelas = '".$id_kelas."' AND id_semester = '".$id_semester."'");
$cekin = mysqli_num_rows($sql);
if ($cekin == 1) {
	echo "<script>
	alert('Silahkan hapus dahulu nilai sikap Sosial sebelumnya');
	...
	//script input
	$query = mysqli_query($koneksi, "INSERT INTO nilai_raport_sosial(id_nilairaport_so, id_siswa, nilai, predikat, deskripsi, id_kelas, id_semester, tgl_raport, tahun_ajaran_awal, tahun_ajaran_akhir) 
		VALUES('".$idrpt."','".$idsiswa."', '".$nilainnya."','".$hasilnya."','".$des."', '".$id_kelas."','".$id_semester."','".$tgl_raport."','".$tahun_awal."','".$tahun_akhir."')");
	if ($query) {
	echo "<script>
